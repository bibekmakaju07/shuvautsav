...
final checkoutStoreProvider = StateNotifierProvider.autoDispose<
    CheckoutStoreController,
    AppState<OrderResponseModel, NetworkFailure>
  >((ref) {
  return CheckoutStoreController();
});

class CheckoutStoreController
    extends StateNotifier<AppState<OrderResponseModel, NetworkFailure>> {
  CheckoutStoreController() : super(AppState.initial());

  Future<void> storeOnCheckout({required Map<String, dynamic> params}) async {
    if (state == AppState.loading(true)) {
      return;
    }
    state = AppState.loading(true);
    try {
      String url = 'https://shuvautsav.com/api/v1/cart/checkout/store';
